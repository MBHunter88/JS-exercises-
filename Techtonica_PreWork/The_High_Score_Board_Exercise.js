...
//Create a function createScoreBoard that creates an object that serves as a high score board. The keys of this object will be the names of the players, the values will be their scores. 
//For testing purposes, you want to directly include one entry in the object. This initial entry should consist of The Best Ever as player name and 1000000 as score.

function createScoreBoard() {
    return {name: "The Best Ever", score: 1000000}
}
...
// This function should remove the entry for the given player from the board and return the board afterwards. If the player was not on the board in the first place, 
...
function updateScore(obj, name, score){
  if(name !== undefined)  { //why can't I set the name to be equal to obj[name]?
  obj[name] += score
...
function normalizeScore(obj) {
  const { score, normalizeFunction } = obj;
